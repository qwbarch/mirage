...
/// Thanks to Piggy and VirusTLNR:
/// https://github.com/VirusTLNR/LethalIntelligence
/// 
/// Thanks to XuXiaolan for helping me create my own asset bundle with the held item animations.
[<AllowNullLiteral>]
type MaskedAnimator() as self =
    inherit NetworkBehaviour()

    let animator = lazy self.transform.GetChild(0).GetChild(3).GetComponent<Animator>()
    let layerIndex = lazy animator.Value.GetLayerIndex "Held Item"

    let random = Random()
    let itemHolder = GameObject "LocalItemHolder"
    ...
    member this.HoldItem struct (item, scrapValue) =
        ignore <| valueTask {
            // Hacky method to fix an issue on the non-host clients.
            do! Task.Delay 2000
            ...
            animator.Value.SetBool(animation, true)
    ...
    member this.FixedUpdate() =
        if isNotNull this.HeldItem then
            ...
            animator.Value.SetLayerWeight(layerIndex.Value, upperBodyAnimationsWeight)
