...
class WhisperModel(ABC):
    ...
    def _init_dependables(self):
        ...
        # Load Pre Processor
        self.preprocessor = LogMelSpectogram(base_path=self.model_path,n_mels=self.n_mels).to(self.device)

        # Load Speech Segmenter
        self.speech_segmenter = SpeechSegmenter(base_path=self.model_path, vad_model=self.vad_model, device=self.device, **self.speech_segmenter_options)
    ...
    def transcribe_with_vad(self, samples_batch, lang_codes=None, tasks=None, initial_prompts=None, batch_size=8):
        ...
        for audio_signal, prompts, seq_len, seg_metadata in self.data_loader(list(map(to_audio_signal, samples_batch)), lang_codes, tasks, initial_prompts, batch_size=batch_size):
...
class WhisperModelCT2(WhisperModel):
    def __init__(self,
        ...
        tokenizer = Tokenizer(tokenizers.Tokenizer.from_file(tokenizer_file), self.model.is_multilingual, model_path)
