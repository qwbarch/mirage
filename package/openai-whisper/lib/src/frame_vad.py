    ...
    def __init__(
        ...
        if self.device == "cpu":
            ...
            self.vad_pp = torch.jit.load(
                os.path.join(base_path, "vad_pp_cpu.ts")
            ).to(self.device)
            self.vad_model = torch.jit.load(
                os.path.join(base_path, "frame_vad_model_cpu.ts")
            ).to(self.device)
        else:
            self.vad_pp = torch.jit.load(
                os.path.join(base_path, "vad_pp_gpu.ts")
            ).to(self.device)
            self.vad_model = torch.jit.load(
                os.path.join(base_path, "frame_vad_model_gpu.ts")
            ).to(self.device)
